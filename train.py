...
alpha_values = [0.00001, 0.0001, 0.001, 0.01]  # learning rate
...
def get_predictions(A2):
    return np.argmax(
        A2, 0
    )  ## returns the index of the highest "certainty" value - this is what the neural network thinks


def get_accuracy(predictions, Y):
    return (
        np.sum(predictions == np.argmax(Y, 0)) / Y.shape[1]
    )  # Adjusted to handle 2D Y
...
def loss(X, Y, A2, W1, W2, lambd):
    ...
    L2_regularization = (
        lambd / (2 * m) * (np.sum(np.square(W1)) + np.sum(np.square(W2)))
...
def find_best_params(best_accuracy):
    for alpha, lambd, iterations, mini_batch_size in itertools.product(
        alpha_values, lambda_values, iteration_values, mini_batches
    ):
        print(
        ...
        ## Train model
        W1, b1, W2, b2 = gradient_descent_with_mini_batches(
            X_train,
            Y_train,
...
def gradient_descent_with_mini_batches(
    X, Y, iterations, learning_rate, lambd, mini_batch_size
):
    W1, b1, W2, b2 = init_params()
    ...
    for i in range(iterations):
        ...
        for minibatch in minibatches:
            ...
            dW1, db1, dW2, db2 = back_prop(
                Z1, A1, Z2, A2, W1, W2, minibatch_x, minibatch_y, lambd
            )
        ...
        if i % 500 == 0:
            ...
            print(
                "Training accuracy: ",
                f"{get_accuracy(get_predictions(A2), minibatch_y) * 100}%",
            )
...
if __name__ == "__main__":
    ...
    #     alpha=learning_rate,
    #     lambd=lambd,
    # )  ## train model on specific parameters
